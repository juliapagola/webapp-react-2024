...
const DetalleProducto = () => {
    ...
    useEffect(() => {
        if (key) {
            axios.get(`https://webapp-react-2024-dsm-default-rtdb.europe-west1.firebasedatabase.app/productos.json?orderBy="$key"&equalTo="${key}"`)
                .then((response) => {
            ...
            <Row>
                <Col md={6}>
                    <div className="imagen-container d-flex justify-content-center align-items-center">
                        <img className="img-fluid" alt={producto.nombre} src={producto.imagen} />
                    </div>
                </Col>
                <Col md={6}>
                    <div className="info-producto mb-4">
                        <h1>DETALLES DEL PRODUCTO - {key}</h1>
                    ...
                    <Button variant="success" size="lg" className="mt-3">Añadir al carrito</Button>
                </Col>
            </Row>
        </Container >
    );
